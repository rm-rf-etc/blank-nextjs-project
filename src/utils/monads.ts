...
export function isSuccess(thing: Result<unknown, unknown>): thing is Success<unknown> {
  return thing.$s
}

export function onResult(
  thing: Result<unknown, string>,
  onSuccess: CallableFunction,
  onFailure: CallableFunction,
): void {
  isSuccess(thing) ? onSuccess(thing.val) : onFailure(thing.val)
}
...
export const fallback = <V, E = string>(thing: Result<V, E>, fallback: V): V => (
  isSuccess(thing) ? thing.val : fallback
)
